...
内置站点适配包括 Linovelib / 哔哩轻小说、Kakuyomu、Syosetu、Novel18、Pixiv 小说与漫画、
Hameln、Alphapolis、18Comic、Bika，以及 Webtoon、漫画柜、漫画呗、拷贝漫画、动漫之家等常见网页阅读器。
...
- 新增 Windows TTS 听书，可连续朗读小说章节并切换原文或译文。
- 设置页可读取、选择、试听并持久化 Windows 已安装声线，支持恢复系统默认声音。
- 链接解析与导入改为后台任务，弹窗可以收起和恢复，任务日志与进度实时显示。
- 扩展 Pixiv 漫画及常见网页漫画阅读器解析，补充 Webtoon、漫画柜、漫画呗、拷贝漫画、动漫之家等通用规则。

改进：

- 增强 18Comic / JM 与 Bika 的失败重试、线路刷新和错误提示。
- 漫画图片增加响应校验、原子写入和损坏文件重下，避免残缺图片被误判为已完成。
- 听书声线配置仅保存在本机；声音被卸载或配置损坏时提供可恢复的错误处理。
- 修复长漫画标题导致的书架卡片溢出，并减少链接导入期间的界面阻塞。

兼容性说明：

- 本版本仅提供 Windows x64 桌面程序。
- TTS 声线来自 Windows 已安装的系统语音包；第三方站点仍可能因登录、区域限制或页面结构变化而暂时不可用。
