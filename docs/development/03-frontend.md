...
2. 读取持久化偏好；
3. 创建 API 客户端；
4. 检查是否已配置远程后端；已配置时执行认证和版本握手，未配置时进入设置页；
...
- 应用不得创建、终止或探测客户端设备上的 Python 进程；连接生命周期只管理 HTTP 客户端和页面轮询。

## 9. Linux 远程连接

连接配置只包含规范化根地址和必需 Token：

- 使用用户配置的 `https://` 地址，或可信私有网络中的 `http://` 地址；必须提供 Token。
- 首次启动没有有效连接时直接选择设置区，并显示连接引导；其他功能保持可见但呈现可操作的未连接状态。
- 连接失败、认证失败或版本不兼容时，只显示对应诊断与重试入口，不得回退到回环地址或本地后端。
