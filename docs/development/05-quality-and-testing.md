...
- 搜索页覆盖默认“书源”、切换“夸克”“番茄”和“起点”、切换时清空旧结果、四种加载文案与统一加入书架载荷；手机窄宽度不得因引擎选择器、
  输入框和搜索按钮同排而溢出。Android 内置搜索结果加入书架必须覆盖异步链接任务的排队、完成、失败与按钮锁定，Windows 保持原有同步导入流程；网关 HTML 错误不得原样出现在提示中。
- Android 详情页在带状态栏安全区的窄屏上验证标题不与系统图标重叠；阅读器进入、控制层显隐和前台恢复都必须保持 Flutter `edgeToEdge`，并通过原生阅读通道请求“隐藏顶部状态栏且关闭 decor 避让”，离开时恢复顶部状态栏。测试必须拒绝手动覆盖层和 `immersiveSticky`，并验证阅读器背景覆盖完整窗口。正文测试必须验证段首标记被转换成固定宽度布局占位而不是空白字符，并通过实际换行后的字形坐标断言：短段和自动换行长段的首个正文字符位于同一两字缩进位置，正文保持两端对齐，换行续行与跨页续行不重复缩进。
...
- Windows 本机后端设置覆盖“打开模型设置”按钮的就绪 / 禁用状态、固定回环 URL、打开失败提示，以及管理界面
  `#settings` 深链接；测试不得真正启动浏览器。
...
- 夸克适配使用固定响应 Fixture 覆盖搜索映射、作品与目录解析、目录短时复用、429 / 临时网关错误有界重试、正文还原、非免费章节拒绝、非官方正文域名拒绝及插件停用短路；
...
- 起点适配使用固定移动站 SSR Fixture 覆盖搜索字段映射、作品链接规范化、二维码状态、目录访问标记、书架分页去重与受保护章节拒绝；搜索不得携带账号 Cookie，CI 不依赖真实登录账号；
- 任务状态与失败恢复；
