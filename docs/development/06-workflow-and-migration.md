...
- 当前已发布基线为 `1.7.5+26`；后续正式发布的 build 必须大于 `26`，不得回退版本或复用已发布 build。
- `1.7.5+26` 已移除参与字体整形的不可见缩进字符并加入顶部状态栏原生通道，但实机回报证明只约束段落内部坐标会漏掉外层短段收缩居中，且部分 OEM 会在窗口重新获焦时重置 Insets。设备无关的统一阅读栏宽度、随文字缩放的首行占位、窗口获焦重应用和状态栏/刘海背景兜底使用 `1.7.6+27`，对外标签为 `v1.7.6`。
