...
void main() {
  test('reader palettes expose unique backgrounds and readable text', () {
    final backgrounds = ReaderPalette.palettes
        .map((palette) => palette.background.value)
        .toSet();
    ...
    expect(
      ReaderPalette.fromMode(ReaderPaletteMode.night).isDark,
      isTrue,
    );
  });
  ...
  test('reader preference labels cover every persisted option', () {
    expect(
      ReaderPageAnimation.values.map((value) => value.label),
      <String>['覆盖', '平移', '淡入', '无动画'],
    );
    expect(
      ReaderLineSpacing.values.map((value) => value.label),
      <String>['紧凑', '标准', '舒展'],
    );
  });
}
